...
All the questions are marked by the question number as a comment from q3-9
...
where  s.SUPP_ID in (select SUPP_ID from supplier_pricing having count(pro_id) > 1);


-- question 6
-- 6)	Find the least expensive product from each category and print the table with category id, name, product name and price of the product

select c.cat_id, c.cat_name,p.pro_name as product_nm, sp.supp_price as min_price
from product p
inner join category c
on c.cat_id = p.cat_id
inner join supplier_pricing sp
on sp.pro_id = p.pro_id
where (c.cat_id, c.cat_name, sp.supp_price ) in (
select c.cat_id, c.cat_name, min(sp.supp_price) as price 
from product p
inner join category c
on c.cat_id = p.cat_id
inner join supplier_pricing sp
...
DELIMITER //
CREATE PROCEDURE GetRatingService()
BEGIN
	select s.supp_id, s.supp_name, r.rat_ratstars,
    case 
    when r.rat_ratstars = 5
		then 'Excellent Service'
	when r.rat_ratstars > 4 and r.rat_ratstars < 5
		then 'Good Service'
	when r.rat_ratstars > 2 and r.rat_ratstars <= 4
